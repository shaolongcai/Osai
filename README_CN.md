...
- ⌨️ **键盘快捷键** - 高效的键盘导航，快速访问文件

## 🛠️ 技术栈

### 前端技术
- **React 19.1** - 现代化的用户界面框架
- **TypeScript 5.8** - 类型安全的 JavaScript 超集
- **Material-UI (MUI) 7.3** - React 组件库
...
**注意**：左右方向键在搜索框有内容时有效。上下方向键和 Enter 键在显示搜索结果时有效。
...
- **`pages/`** - 页面组件
  ...
  - `Setting.tsx` - 设置页面
- **`contexts/`** - 上下文管理
  - `I18nContext.tsx` - 国际化上下文(通过 i18n 模块)
  - `globalContext.ts` - 全局状态上下文
- **`hooks/`** - 自定义 React Hooks
  - `useIcon.ts` - 图标 Hook
...
- **JSON 翻译文件**: 支持 8 种语言的完整翻译，位于 `frontend/src/i18n/locales/` 目录下
...
1. 在 `frontend/src/i18n/locales/` 中添加新的语言 JSON 文件(例如 `xx-XX.json`)
2. 在 `frontend/src/i18n/index.tsx` 中添加语言配置(在 `SUPPORTED_LANGUAGES` 数组中)
...
- **项目主页**: [GitHub Repository](https://github.com/shaolongcai/Osai)
- **问题反馈**: [GitHub Issues](https://github.com/shaolongcai/Osai/issues)
- **讨论交流**: [GitHub Discussions](https://github.com/shaolongcai/Osai/discussions)
