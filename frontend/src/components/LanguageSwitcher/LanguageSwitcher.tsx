...
const LanguageSwitcher: React.FC<LanguageSwitcherProps> = ({ 
  ...
  const getLanguageText = (languageCode: Language): string => {
    // 使用翻譯函數獲取語言的本地化名稱
    const translationKey = `app.language.${languageCode}` as TranslationKeyPath;
   ...
   */
  const getButtonColor = (language: Language): 'primary' | 'inherit' => {
  ...
  const renderLanguageButtons = () => {
    return languages.map((lang) => (
      <Button
        ...
        color={getButtonColor(lang)}           // 按鈕顏色主題
